"""
AI Fingerprint Detector v6.0.0 — Hybrid Neural-Structural Strict Analysis
...
AI_ENGINE_VERSION = "6.0.0 Hybrid Neural-Structural Academic Engine"
_LOCAL_NEURAL_MODEL = Path(__file__).resolve().parent / "models" / "tmr-ai-text-detector"
NEURAL_MODEL_ID = os.getenv(
    "AI_DETECTOR_MODEL",
    str(_LOCAL_NEURAL_MODEL) if _LOCAL_NEURAL_MODEL.exists() else "Oxidane/tmr-ai-text-detector",
)
...
def _load_neural_detector():
    """Load a memory-efficient ONNX detector, with PyTorch as a fallback."""
    ...
    if prefer_onnx and ONNX_OK and HF_HUB_OK:
        try:
            local_onnx = _LOCAL_NEURAL_MODEL / NEURAL_ONNX_FILENAME
            if local_onnx.exists():
                model_path = str(local_onnx)
                tokenizer_source = str(_LOCAL_NEURAL_MODEL)
            else:
                model_path = hf_hub_download(
                    repo_id=NEURAL_ONNX_REPO,
                    filename=NEURAL_ONNX_FILENAME,
                )
                tokenizer_source = NEURAL_ONNX_REPO
            ...
            if not TORCH_OK:
                raise NeuralDetectorUnavailableError(
                    "تعذر فتح نموذج ONNX INT8، ولا يوجد PyTorch كمسار احتياطي. "
                    f"Model={NEURAL_ONNX_REPO}/{NEURAL_ONNX_FILENAME}; details={exc}"
                ) from exc
    ...
    if TORCH_OK:
        try:
            try:
                ...
                pass
            tokenizer = AutoTokenizer.from_pretrained(NEURAL_MODEL_ID, use_fast=True)
            model = AutoModelForSequenceClassification.from_pretrained(NEURAL_MODEL_ID)
            model.to("cpu")
            model.eval()
            return tokenizer, model, "pytorch"
        except Exception as exc:
            raise NeuralDetectorUnavailableError(
                "تعذر تنزيل أو فتح نموذج الكشف المتعلّم. تأكد من اتصال المنصة بالإنترنت "
                "ومن وجود مساحة ذاكرة كافية في أول تشغيل. "
                f"Model={NEURAL_MODEL_ID}; details={exc}"
            ) from exc
...
_banner_msg = (
    "🎉 <b>الإصدار 6.0.0:</b> محرك هجين بمصنف متعلّم وتحليل بنيوي "
