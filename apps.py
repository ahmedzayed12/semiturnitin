...
# نموذج واحد ثابت لتقليل عدد المسارات والطلبات على الواجهة البرمجية.
# لا يجرّب البرنامج أي نموذج بديل، ولا يصدر التقرير إلا بعد نجاح هذا النموذج.
GEMINI_FIXED_MODEL = "gemini-2.5-flash"
GEMINI_DEFAULT_MODEL = GEMINI_FIXED_MODEL
...
def _configured_gemini_model() -> str:
    """إرجاع النموذج الثابت فقط؛ لا تبديل ولا اكتشاف ولا تجريب لنماذج أخرى."""
...
def _gemini_rest_generate(api_key: str, model: str, prompt: str,
    ...
    if model != GEMINI_FIXED_MODEL:
        raise GeminiAPIError("إعداد نموذج المراجعة الذكية غير صحيح. يجب استخدام النموذج الثابت المعتمد.")

    endpoint = (
        "https://generativelanguage.googleapis.com/v1beta/models/"
        + urllib.parse.quote(GEMINI_FIXED_MODEL, safe="-._")
        + ":generateContent"
    )
    payload = {
        "system_instruction": {
    ...
    for attempt in range(GEMINI_MAX_ATTEMPTS):
        ...
        try:
            ...
            return parsed, GEMINI_FIXED_MODEL
...
def _gemini_assisted_analysis(analysis_text: str, local_result: dict,
    ...
    """تنفيذ مراجعة واحدة إلزامية بالنموذج الثابت؛ لا توجد نماذج بديلة أو طلب اختبار منفصل."""
    if preferred_model != GEMINI_FIXED_MODEL:
        raise GeminiAPIError("إعداد النموذج لا يطابق النموذج الثابت المعتمد.")
