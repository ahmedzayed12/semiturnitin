...
GEMINI_MAX_PASSAGES = 12
GEMINI_MAX_WORDS_PER_PASSAGE = 230
GEMINI_API_TIMEOUT_SECONDS = 150
...
def _configured_gemini_model() -> str:
    model = str(_secret_or_env("GEMINI_MODEL", GEMINI_DEFAULT_MODEL) or GEMINI_DEFAULT_MODEL).strip()
    return model if model in GEMINI_MODEL_OPTIONS else GEMINI_DEFAULT_MODEL
    ...
    for attempt in range(3):
        try:
            with urllib.request.urlopen(
                request, timeout=GEMINI_API_TIMEOUT_SECONDS, context=_gemini_ssl_context()
            ) as response:
                response_data = json.loads(response.read().decode("utf-8"))
            candidates = response_data.get("candidates") or []
            if not candidates:
                feedback = response_data.get("promptFeedback") or {}
                raise GeminiAPIError(f"لم تُرجع خدمة المراجعة الذكية نتيجة. {feedback}")
            parts = (((candidates[0] or {}).get("content") or {}).get("parts") or [])
            raw_text = "\n".join(str(part.get("text") or "") for part in parts if isinstance(part, dict))
            return _extract_json_object(raw_text), model
            ...
            try:
                ...
                detail = str(exc)
            last_error = detail
            if exc.code in (429, 500, 503, 504) and attempt < 2:
            ...
            if exc.code == 403:
                raise GeminiAPIError("بيانات اتصال خدمة المراجعة الذكية غير صالحة أو غير مخوّلة.")
            if exc.code == 429:
                raise GeminiAPIError("تم تجاوز حد استخدام خدمة المراجعة الذكية. أعد المحاولة لاحقًا.")
            if exc.code == 404:
                raise GeminiAPIError("نموذج المراجعة الذكية غير متاح ضمن إعدادات المشروع الحالية.")
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
    raise GeminiAPIError(f"تعذّر الاتصال بخدمة المراجعة الذكية: {last_error or 'unknown error'}")
...
_RISK_CATEGORY_LABELS = {
    "formulaic_language": ("صياغة نمطية", "Formulaic language"),
    "unsupported_claim": ("ادعاءات تحتاج دعماً", "Unsupported claims"),
    "citation_problem": ("دعم الاستشهادات", "Citation support"),
    "style_discontinuity": ("انقطاع أسلوبي", "Style discontinuity"),
    "overgeneralization": ("تعميم زائد", "Overgeneralization"),
    "possible_ai_assistance": ("مؤشرات مساعدة آلية", "Automated-assistance signals"),
    "other": ("مؤشرات أخرى", "Other review signals"),
}


def _academic_risk_compass(local_result: dict, review: dict, combined: dict | None = None) -> dict:
    """Build a practical risk map from both analysis sources without another API call."""
    combined = combined or _combined_probability(local_result, review)
    category_points = collections.Counter()
    category_hits = collections.Counter()
    severity_total = 0
    flags = review.get("flagged_passages") or []
    for flag in flags:
        category = str(flag.get("category") or "other")
        if category not in _RISK_CATEGORY_LABELS:
            category = "other"
        try:
            severity = max(1, min(5, int(flag.get("severity", 1))))
        except Exception:
            severity = 1
        category_points[category] += severity
        category_hits[category] += 1
        severity_total += severity

    # Citation risk is important even when the service returned no individual citation flag.
    citation_risk = max(0.0, min(100.0, float(review.get("citation_integrity_risk_score", 0.0))))
    if citation_risk > 0:
        category_points["citation_problem"] += citation_risk / 20.0

    score_items = []
    for key, points in category_points.items():
        score = int(round(max(0.0, min(100.0, points * 18.0))))
        ar, en = _RISK_CATEGORY_LABELS[key]
        score_items.append({
            "key": key,
            "label_ar": ar,
            "label_en": en,
            "score": score,
            "hits": int(category_hits.get(key, 0)),
        })
    score_items.sort(key=lambda item: (item["score"], item["hits"]), reverse=True)

    dominant = score_items[0] if score_items else {
        "key": "other", "label_ar": "لا توجد بؤرة خطر واضحة",
        "label_en": "No dominant risk area", "score": 0, "hits": 0,
    }
    avg_severity = severity_total / max(len(flags), 1) if flags else 0.0
    final_score = max(0.0, min(100.0, float(combined.get("score", 0.0))))
    evidence = max(0.0, min(100.0, float(local_result.get("confidence_percentage", 0.0))))
    defensibility = 100.0 - (
        0.42 * final_score + 0.28 * citation_risk +
        0.18 * (avg_severity / 5.0 * 100.0) + 0.12 * (100.0 - evidence)
    )
    defensibility = int(round(max(0.0, min(100.0, defensibility))))
    if defensibility >= 75:
        defensibility_label_ar = "قابلية دفاع مرتفعة"
        defensibility_label_en = "High defensibility"
    elif defensibility >= 50:
        defensibility_label_ar = "قابلية دفاع متوسطة"
        defensibility_label_en = "Moderate defensibility"
    else:
        defensibility_label_ar = "يحتاج تدقيقاً موسعاً"
        defensibility_label_en = "Extended review recommended"

    return {
        "available": True,
        "dominant_risk_ar": dominant["label_ar"],
        "dominant_risk_en": dominant["label_en"],
        "dominant_risk_score": dominant["score"],
        "categories": score_items[:6],
        "flag_count": len(flags),
        "average_severity": round(avg_severity, 1),
        "academic_defensibility_score": defensibility,
        "academic_defensibility_label_ar": defensibility_label_ar,
        "academic_defensibility_label_en": defensibility_label_en,
        "focus_ar": (
            f"ابدأ المراجعة من: {dominant['label_ar']}، ثم تحقق من الاستشهادات والعبارات المظللة."
            if score_items else
            "لم تظهر بؤرة خطر محددة؛ راجع العبارات المظللة مع السياق الأكاديمي الكامل."
        ),
        "focus_en": (
            f"Start with {dominant['label_en'].lower()}, then verify citations and highlighted passages."
            if score_items else
            "No dominant risk area was found; review highlighted passages in full academic context."
        ),
    }



_REVISION_ACTIONS_AR = {
    "formulaic_language": "أضف تفاصيل خاصة ببحثك أو بيانات فعلية، وقلّل العبارات العامة الجاهزة مع الحفاظ على المعنى العلمي.",
    "unsupported_claim": "اربط الادعاء بدليل أو مرجع مناسب، أو قيّده بحدود العينة والنتائج المتاحة.",
    "citation_problem": "تحقق من أن المرجع يدعم الادعاء مباشرة، وأضف رقم الصفحة أو المصدر الأقرب عند الحاجة.",
    "style_discontinuity": "وحّد المصطلحات ونبرة الكتابة مع الفقرات السابقة واللاحقة، ثم راجع الانتقال المنطقي بينها.",
    "overgeneralization": "استبدل التعميم بصياغة محددة توضّح المجال والعينة والظروف التي تنطبق عليها النتيجة.",
    "possible_ai_assistance": "راجع الحقائق والمصطلحات يدويًا، وأضف تحليلك الأصلي وتفسيرك للنتيجة بدل الاكتفاء بصياغة عامة.",
    "other": "راجع المقطع في سياقه، وحدد الادعاء الأساسي، ثم حسّن الدليل والصياغة والاتصال بما قبله وما بعده.",
}

_REVISION_ACTIONS_EN = {
    "formulaic_language": "Add study-specific details or actual evidence and reduce generic stock phrasing while preserving the scientific meaning.",
    "unsupported_claim": "Support the claim with appropriate evidence or narrow it to the limits of the sample and available results.",
    "citation_problem": "Verify that the citation directly supports the claim and add the closest source or page reference when appropriate.",
    "style_discontinuity": "Align terminology and voice with adjacent sections and improve the logical transition.",
    "overgeneralization": "Replace the broad statement with a qualified claim that states the population, scope, and conditions.",
    "possible_ai_assistance": "Manually verify facts and terminology, then add the author's original analysis and interpretation.",
    "other": "Review the passage in context, identify its central claim, and improve its evidence, wording, and connection to surrounding text.",
}


def _build_smart_revision_plan(review: dict) -> dict:
    """حوّل نتائج المراجعة إلى خطوات عملية مفهومة للمستخدم."""
    items = []
    flags = list(review.get("flagged_passages") or [])
    flags.sort(key=lambda item: int(item.get("severity", 1) or 1), reverse=True)
    for idx, flag in enumerate(flags[:10], start=1):
        category = str(flag.get("category") or "other")
        if category not in _REVISION_ACTIONS_AR:
            category = "other"
        try:
            severity = max(1, min(5, int(flag.get("severity", 1))))
        except Exception:
            severity = 1
        if severity >= 4:
            priority_ar, priority_en = "عاجلة", "High"
        elif severity == 3:
            priority_ar, priority_en = "متوسطة", "Medium"
        else:
            priority_ar, priority_en = "تحسينية", "Improvement"
        items.append({
            "number": idx,
            "passage_id": str(flag.get("passage_id") or f"P{idx:02d}"),
            "severity": severity,
            "priority_ar": priority_ar,
            "priority_en": priority_en,
            "category": category,
            "category_ar": _RISK_CATEGORY_LABELS.get(category, _RISK_CATEGORY_LABELS["other"])[0],
            "category_en": _RISK_CATEGORY_LABELS.get(category, _RISK_CATEGORY_LABELS["other"])[1],
            "reason_ar": str(flag.get("reason_ar") or "يحتاج إلى مراجعة بشرية.").strip(),
            "reason_en": str(flag.get("reason_en") or "Human review is recommended.").strip(),
            "quote": str(flag.get("quote") or "").strip(),
            "action_ar": _REVISION_ACTIONS_AR[category],
            "action_en": _REVISION_ACTIONS_EN[category],
        })
    general_ar = list(review.get("recommended_actions_ar") or [])[:5]
    general_en = list(review.get("recommended_actions_en") or [])[:5]
    return {
        "available": True,
        "items": items,
        "urgent_count": sum(1 for item in items if item["severity"] >= 4),
        "general_actions_ar": general_ar,
        "general_actions_en": general_en,
        "checklist_ar": [
            "راجعت كل مقطع ذي أولوية عاجلة في سياقه الكامل.",
            "تحققت من أن كل ادعاء مهم تدعمه نتيجة أو إحالة مناسبة.",
            "وحّدت المصطلحات والأسلوب بين أقسام البحث.",
            "أضفت تحليلي وتفسيري الأصلي بدل العبارات العامة.",
            "أجريت قراءة نهائية للمراجع والأرقام والأسماء قبل التسليم.",
        ],
        "checklist_en": [
            "Reviewed every high-priority passage in full context.",
            "Verified that each important claim is supported by evidence or a suitable citation.",
            "Aligned terminology and writing style across sections.",
            "Added original analysis and interpretation instead of generic statements.",
            "Completed a final check of references, numbers, names, and factual details.",
        ],
    }
...
def _gemini_assisted_analysis(analysis_text: str, local_result: dict,
    ...
    models = [preferred_model] + [m for m in GEMINI_MODEL_OPTIONS if m != preferred_model]
    errors = []
    for model in models:
        try:
            raw, used_model = _gemini_rest_generate(api_key, model, prompt)
            return _normalize_gemini_review(raw, passages, used_model)
        except GeminiAPIError as exc:
            errors.append(str(exc))
            if "غير صالح" in str(exc) or "الخطة المجانية" in str(exc):
                break
    raise GeminiAPIError(" | ".join(dict.fromkeys(errors)) or "تعذرت المراجعة الذكية.")


def _gemini_connection_test(api_key: str, model: str) -> str:
    schema = {
        "type": "object",
        "properties": {"status": {"type": "string"}},
        "required": ["status"],
    }
    result, used_model = _gemini_rest_generate(
        api_key, model,
        'Return only JSON with status set to "ok". Do not add any other fields.',
        response_schema=schema,
    )
    if str(result.get("status") or "").lower() != "ok":
        raise GeminiAPIError("استجابة اختبار الاتصال غير متوقعة.")
    return used_model
...
def generate_report_pdf_from_original(original_pdf_bytes: bytes,
    ...
    revision_buf = io.BytesIO()
    cv4 = rl_canvas.Canvas(revision_buf, pagesize=A4)
    _draw_smart_revision_plan_page(cv4, W_pt, H_pt, result, doc_name, today)
    cv4.save()
    revision_buf.seek(0)
    revision_doc = fitz.open(stream=revision_buf.read(), filetype="pdf")
    out_doc.insert_pdf(revision_doc)
...
def _draw_summary_page(c, W, H, result, doc_name, doc_date):
    ...
    _risk_compass = result.get('academic_risk_compass') or _academic_risk_compass(
        result, _summary_review, _summary_combined
    )
    metrics = [
        ("FINAL SCREENING SCORE", _format_percentage(_summary_score)),
        ("English words analyzed", f"{result.get('n_words', 0):,}"),
        ("Academic defensibility", f"{_risk_compass.get('academic_defensibility_score', 0)}/100"),
        ("Dominant risk area", _risk_compass.get('dominant_risk_en', 'None')[:28]),
...
def _draw_gemini_review_page(c, W, H, result, doc_name, doc_date):
    ...
    risk_compass = result.get('academic_risk_compass') or _academic_risk_compass(
        result, review, result.get('combined_analysis') or {}
    )
    y = draw_wrapped('Assessment summary', review.get('summary_en', ''), y, max_lines=7)
    y = draw_wrapped(
        'Academic Risk Compass',
        f"Academic defensibility: {risk_compass.get('academic_defensibility_score', 0)}/100 | "
        f"Dominant area: {risk_compass.get('dominant_risk_en', 'None')}. "
        f"{risk_compass.get('focus_en', '')}",
        y, max_lines=5
    )
...
def _draw_smart_revision_plan_page(c, W, H, result, doc_name, doc_date):
    """آخر صفحة بالتقرير: خطة مراجعة عملية وقائمة تحقق قبل التسليم."""
    review = result.get("gemini_review") or {}
    plan = result.get("smart_revision_plan") or _build_smart_revision_plan(review)
    c.setFillColor(HexColor('#f8fbff'))
    c.rect(0, 0, W, H, fill=1, stroke=0)
    c.setFillColor(HexColor('#060d1a'))
    c.rect(0, H - 46, W, 46, fill=1, stroke=0)
    c.setFillColor(HexColor('#27ae60'))
    c.rect(0, H - 47.5, W, 1.5, fill=1, stroke=0)
    c.setFillColor(HexColor('#ffffff'))
    c.setFont('Helvetica-Bold', 14)
    c.drawString(38, H - 29, 'Smart Revision Plan')
    c.setFillColor(HexColor('#a9c5b3'))
    c.setFont('Helvetica', 7.5)
    c.drawRightString(W - 38, H - 28, 'Actionable steps before submission')

    y = H - 76
    items = plan.get('items') or []
    c.setFillColor(HexColor('#e8f5e9'))
    c.roundRect(38, y - 42, W - 76, 42, 6, fill=1, stroke=0)
    c.setStrokeColor(HexColor('#27ae60'))
    c.roundRect(38, y - 42, W - 76, 42, 6, fill=0, stroke=1)
    c.setFillColor(HexColor('#1b5e20'))
    c.setFont('Helvetica-Bold', 9.5)
    c.drawString(48, y - 16, f"Priority passages: {len(items)}  |  High-priority actions: {plan.get('urgent_count', 0)}")
    c.setFont('Helvetica', 8)
    c.drawString(48, y - 31, 'Use this page as a revision checklist; it does not automatically rewrite the author\'s work.')
    y -= 62
    ...
    if items:
        for item in items[:5]:
            ...
            box_h = 86
            c.setFillColor(HexColor('#ffffff'))
            c.roundRect(38, y - box_h, W - 76, box_h, 6, fill=1, stroke=0)
            border = '#c0392b' if item.get('severity', 1) >= 4 else '#f4a300' if item.get('severity', 1) == 3 else '#00a8cc'
            c.setStrokeColor(HexColor(border))
            c.roundRect(38, y - box_h, W - 76, box_h, 6, fill=0, stroke=1)
            c.setFillColor(HexColor(border))
            c.setFont('Helvetica-Bold', 8.5)
            c.drawString(48, y - 15, f"[ ] {item.get('passage_id')}  |  {item.get('priority_en')} priority  |  {item.get('category_en')}")
            c.setFillColor(HexColor('#334e68'))
            c.setFont('Helvetica', 7.4)
            reason_lines = textwrap.wrap('Why: ' + str(item.get('reason_en') or ''), width=100)[:2]
            yy = y - 29
            for line in reason_lines:
                c.drawString(48, yy, line); yy -= 10
            action_lines = textwrap.wrap('Action: ' + str(item.get('action_en') or ''), width=100)[:2]
            for line in action_lines:
                c.drawString(48, yy, line); yy -= 10
            quote = str(item.get('quote') or '')
            if quote:
                quote_lines = textwrap.wrap('Excerpt: "' + quote[:210] + '"', width=100)[:2]
                for line in quote_lines:
                    c.setFillColor(HexColor('#607d8b'))
                    c.drawString(48, yy, line); yy -= 9
            y -= box_h + 9
    else:
        c.setFillColor(HexColor('#ffffff'))
        c.roundRect(38, y - 58, W - 76, 58, 6, fill=1, stroke=0)
        c.setStrokeColor(HexColor('#b7d7c3'))
        c.roundRect(38, y - 58, W - 76, 58, 6, fill=0, stroke=1)
        c.setFillColor(HexColor('#2e5d3b'))
        c.setFont('Helvetica-Bold', 9)
        c.drawString(48, y - 22, 'No passage-specific corrective actions were generated.')
        c.setFont('Helvetica', 8)
        c.drawString(48, y - 39, 'Complete the final quality checklist below before submission.')
        y -= 76

    checklist = plan.get('checklist_en') or []
    start_y = min(y - 8, 225)
    c.setFillColor(HexColor('#12263a'))
    c.setFont('Helvetica-Bold', 10)
    c.drawString(38, start_y, 'Final pre-submission checklist')
    cy = start_y - 17
    c.setFillColor(HexColor('#334e68'))
    c.setFont('Helvetica', 8)
    for item in checklist[:5]:
        for j, line in enumerate(textwrap.wrap(str(item), width=96)[:2]):
            prefix = '[ ] ' if j == 0 else '    '
            c.drawString(46, cy, prefix + line)
            cy -= 11
        cy -= 2
    ...
    c.setFillColor(HexColor('#8aa0b8'))
    c.setFont('Helvetica', 7)
    c.drawCentredString(W/2, 8, 'Smart Revision Plan — Human review remains essential')


def _para_is_suspicious(para: str, analysis_result: dict | None = None) -> bool:
    """
    يحدد إذا كانت الفقرة مشبوهة (AI) بناءً على نفس منهجية التظليل الأصلية.
    يُستخدم في تقرير DOCX/النص المباشر فقط.
    """
    if _text_matches_review_flag(para, analysis_result):
        return True
...
def generate_report_pdf_text_only(text: str, doc_name: str = "Document",
    ...
    c.showPage()
    _draw_smart_revision_plan_page(c, W, H, result, doc_name, today)
    c.save()
    return buf.getvalue()
...
<div class="features-wrap">
    ...
    <div class="features-grid">
        ...
        <div class="feature-card">
            <div class="feature-icon">✅</div>
            <strong>خطة المراجعة الذكية</strong>
            <span>لا يكتفي التقرير بالتظليل؛ بل يضيف في نهايته قائمة مرتبة بالمقاطع الأهم، وسبب المراجعة، والإجراء الأكاديمي المقترح، مع قائمة تحقق جاهزة قبل التسليم.</span>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
st.info("📌 يفحص البرنامج النصوص الإنجليزية بمحرك داخلي متعدد المؤشرات ومراجعة مدعومة من الذكاء الاصطناعي. تعرض الواجهة نسبة النص المحتمل صياغته بمساعدة الذكاء الاصطناعي؛ وتُخفى النسب من 0 إلى 20 بصيغة *% دون تظليل، ويبدأ التظليل وإظهار الرقم من 21% فأعلى.")
...
if analyze_btn and input_text and input_text.strip():
    ...
    if not _GEMINI_SERVICE_ENABLED:
        ...
        try:
            with st.spinner("جارٍ تنفيذ الفحص الداخلي والمراجعة المدعومة من الذكاء الاصطناعي..."):
                ...
                result["academic_risk_compass"] = _academic_risk_compass(
                    result, result["gemini_review"], result["combined_analysis"]
                )
                result["smart_revision_plan"] = _build_smart_revision_plan(result["gemini_review"])
...
if "last_result" in st.session_state:
    ...
    risk_compass = r.get("academic_risk_compass") or _academic_risk_compass(r, gemini_review, combined)
    st.markdown("#### 🧭 بوصلة المخاطر الأكاديمية")
    _rk1, _rk2, _rk3 = st.columns(3)
    _rk1.metric("قابلية الدفاع الأكاديمي", f"{risk_compass.get('academic_defensibility_score', 0)}/100")
    _rk2.metric("أعلى بؤرة للمراجعة", risk_compass.get("dominant_risk_ar", "غير محددة"))
    _rk3.metric("متوسط شدة العلامات", f"{risk_compass.get('average_severity', 0):.1f}/5")
    st.caption(risk_compass.get("focus_ar", ""))
    if risk_compass.get("categories"):
        with st.expander("تفاصيل خريطة المخاطر"):
            for _risk in risk_compass["categories"]:
                st.progress(_risk.get("score", 0) / 100.0, text=f"{_risk.get('label_ar')}: {_risk.get('score', 0)}/100")

    revision_plan = r.get("smart_revision_plan") or _build_smart_revision_plan(gemini_review)
    st.markdown("#### ✅ خطة المراجعة الذكية قبل التسليم")
    st.caption("هذه الخدمة تحول المقاطع المحددة إلى خطوات عملية واضحة، وتظهر كآخر صفحة في التقرير القابل للتحميل.")
    if revision_plan.get("items"):
        with st.expander("عرض المقاطع والإجراءات المقترحة", expanded=True):
            for _item in revision_plan["items"][:10]:
                st.markdown(
                    f"**☐ {_item.get('passage_id')} — أولوية {_item.get('priority_ar')} — {_item.get('category_ar')}**  \n"
                    f"**سبب المراجعة:** {_item.get('reason_ar')}  \n"
                    f"**الإجراء المقترح:** {_item.get('action_ar')}"
                )
                if _item.get("quote"):
                    st.code(_item.get("quote"), language=None)
    else:
        st.success("لم تُولد إجراءات خاصة بمقاطع بعينها؛ ستظهر قائمة التحقق النهائية في آخر التقرير.")
    with st.expander("قائمة التحقق النهائية"):
        for _check in revision_plan.get("checklist_ar", []):
