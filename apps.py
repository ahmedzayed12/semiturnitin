...
def _draw_summary_page(c, W, H, result, doc_name, doc_date):
    ...
    c.drawRightString(W - 18, H - 24, f"Summary  |  Combined screening likelihood: {int(round(float(_summary_score)))}%")
...
def _draw_gemini_review_page(c, W, H, result, doc_name, doc_date):
    ...
    for i, (label, value, color) in enumerate([
        ('AI-assistance estimate', f'{ai_score}/100', '#c0392b'),
...
st.info("📌 يفحص البرنامج النصوص الإنجليزية عبر محرك داخلي ومراجعة Gemini مدمجة. النسبة النهائية مؤشر احتمالي للمراجعة وليست إثباتًا قاطعًا أو نسبة اقتباس.")

_gemini_consent = st.checkbox(
    "أوافق على إرسال مقتطفات مختارة من متن البحث إلى خدمة المراجعة الذكية.",
    value=False,
    key="gemini_data_consent",
    help="لا تُرسل صفحة المراجع ولا الملف كاملًا؛ تُرسل مقتطفات مختارة فقط.",
)
...
if "last_result" in st.session_state:
    st.markdown("### النتيجة الاحتمالية المركبة")
    ...
    st.warning(
        "النسبة الظاهرة مؤشر احتمالي مركب من المحرك الداخلي ومراجعة Gemini. "
        "لا تمثل إثباتًا قاطعًا لهوية الكاتب ولا نسبة اقتباس أو سرقة علمية."
    )

    _pc1, _pc2, _pc3 = st.columns(3)
    _pc1.metric("المؤشر الداخلي", f"{combined.get('local_score', local_pct):.0f}%")
    _pc2.metric("تقدير Gemini", f"{combined.get('gemini_score', 0):.0f}%")
    ...
    if gemini_review.get("available"):
        st.markdown("#### تفسير المراجع الذكي")
        st.write(gemini_review.get("summary_ar", ""))
        _gc1, _gc2, _gc3, _gc4 = st.columns(4)
        _gc1.metric("تقدير المساعدة بالذكاء الاصطناعي", f"{gemini_review.get('ai_assistance_likelihood_score', 0)}/100")
