...
# ══════════════════════════════════════════════════════════════════════════════
# The API key is never stored in this file. It is read from Streamlit Secrets,
# an environment variable, or a temporary password field in the current session.
...
def _get_gemini_api_key() -> str:
    """Read the key without logging or persisting it in the source code."""
    value = ""
    try:
        value = str(st.secrets.get("GEMINI_API_KEY", "") or "").strip()
    except Exception:
        value = ""
    if not value:
        value = str(os.environ.get("GEMINI_API_KEY", "") or "").strip()
    if not value:
        value = str(st.session_state.get("gemini_api_key_input", "") or "").strip()
    return value
...
_GEMINI_RESPONSE_SCHEMA = {
    ...
    "required": [
        "review_priority_score", "citation_integrity_risk_score", "confidence",
...
5. The review-priority score is a triage score (0-100), not a probability that AI wrote the paper.
6. Flag only supplied passage IDs. Quotes must be copied exactly from the supplied passage and kept under 180 characters.
7. Use conservative judgments. A high score requires repeated, concrete evidence across several passages.
8. Return Arabic explanations for the interface and English explanations for the PDF report.
...
def _extract_json_object(raw_text: str) -> dict:
    ...
    if start >= 0 and end > start:
        ...
        if isinstance(value, dict):
            return value
    raise GeminiAPIError("أعاد Gemini استجابة غير قابلة للقراءة بصيغة JSON.")
    ...
    for attempt in range(3):
        try:
            ...
            if not candidates:
                ...
                raise GeminiAPIError(f"لم يُرجع Gemini نتيجة. {feedback}")
            ...
            if exc.code == 400 and "free tier is not available" in detail.lower():
                raise GeminiAPIError("الخطة المجانية لـGemini غير متاحة في منطقة المشروع الحالية؛ فعّل Billing أو استخدم مشروعًا مدعومًا.")
            if exc.code == 403:
                raise GeminiAPIError("مفتاح Gemini غير صالح أو لا يملك صلاحية الوصول إلى النموذج.")
            if exc.code == 429:
                raise GeminiAPIError("تم تجاوز حد الاستخدام المجاني لـGemini. انتظر قليلًا ثم أعد المحاولة.")
            if exc.code == 404:
                raise GeminiAPIError(f"النموذج {model} غير متاح لهذا المشروع.")
            raise GeminiAPIError(f"فشل اتصال Gemini (HTTP {exc.code}).")
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
    raise GeminiAPIError(f"تعذّر الاتصال بخدمة Gemini: {last_error or 'unknown error'}")
...
def _normalize_gemini_review(raw: dict, passages: list[dict], model: str) -> dict:
    ...
    return {
        ...
        "summary_ar": str(raw.get("summary_ar") or "لم يقدم Gemini ملخصًا.").strip()[:1800],
        ...
        "flagged_passages": normalized_flags[:8],
    ...
    for model in models:
        try:
            ...
            if "غير صالح" in str(exc) or "الخطة المجانية" in str(exc):
                break
    raise GeminiAPIError(" | ".join(errors) or "تعذرت مراجعة Gemini.")
...
def _draw_cover_rl(c, W, H, result, doc_name, doc_date):
    ...
    pct = float(gemini_review.get('review_priority_score', local_pct))
    display_color = gemini_review.get('color') or result['color']
    ...
    c.drawCentredString(W/2, H - 170, "Gemini-Assisted Academic Review")
    c.setFillColor(HexColor('#7fb3c8'))
    c.setFont("Helvetica", 11)
    c.drawCentredString(W/2, H - 190, "Review Priority and Citation-Support Screening")
    ...
    c.drawCentredString(cx, cy - 16, f"{int(round(pct))}")
    c.setFillColor(HexColor('#aaaaaa'))
    c.setFont("Helvetica", 9)
    c.drawCentredString(cx, cy + 30, "Review Priority / 100")
    ...
    for i, (lbl, val, vc) in enumerate([
        ...
        ("Gemini flags", f"{len(gemini_review.get('flagged_passages') or [])}", '#f4a300'),
...
def _draw_summary_page(c, W, H, result, doc_name, doc_date):
    ...
    _summary_score = _summary_review.get('review_priority_score', result['percentage'])
    c.drawRightString(W - 18, H - 24, f"Summary  |  Review priority: {int(round(float(_summary_score)))}/100")
    ...
    metrics = [
        ...
        ("Gemini passages reviewed", f"{_summary_review.get('passages_reviewed', 0):,}"),
        ("Gemini review priority", f"{int(round(float(_summary_score)))}/100"),
        ...
        ("Local evidence sufficiency", f"{result.get('confidence_percentage', 0):.0f}%"),
    ]
    box_w = (W - 116) / 2
    box_h = 58
...
def _draw_gemini_review_page(c, W, H, result, doc_name, doc_date):
    ...
    c.drawString(40, H - 28, 'Gemini-Assisted Academic Review')
    c.setFillColor(HexColor('#9db4c4'))
    c.setFont('Helvetica', 7.5)
    c.drawRightString(W - 40, H - 27, f"Model: {review.get('model', 'Gemini')}")
    ...
    y = draw_wrapped('Assessment summary', review.get('summary_en', ''), y, max_lines=8)
    y = draw_wrapped('Key reasons', review.get('key_reasons_en', []), y, bullet=True, max_lines=10)
    y = draw_wrapped('Recommended actions', review.get('recommended_actions_en', []), y, bullet=True, max_lines=8)
    ...
    c.drawString(48, 50, 'Only selected excerpts were sent to Gemini; references were excluded before review.')
...
with st.sidebar:
    ...
    _secret_key_present = bool(_get_gemini_api_key())
    if _secret_key_present:
        st.success("المفتاح متصل بصورة آمنة.")
    else:
        st.warning("أضف GEMINI_API_KEY في Streamlit Secrets، أو أدخله مؤقتًا هنا للاختبار.")
        st.text_input(
            "Gemini API Key",
            type="password",
            key="gemini_api_key_input",
            placeholder="AIza... أو AQ...",
            help="لا يُحفظ داخل ملف التطبيق أو GitHub.",
        )

    _gemini_model = st.selectbox(
        "نموذج Gemini",
        options=list(GEMINI_MODEL_OPTIONS),
        index=0,
        key="gemini_model_selector",
        help="يبدأ التطبيق بالنموذج المختار ثم يجرب البدائل المتاحة تلقائيًا عند عدم توفره.",
    )
    _gemini_consent = st.checkbox(
        "أوافق على إرسال مقتطفات مختارة من متن البحث إلى Google Gemini للتحليل.",
        value=False,
        key="gemini_data_consent",
    )
    st.caption("لا تُرسل صفحة المراجع، ولا يُرسل الملف الكامل؛ تُرسل مقتطفات مختارة فقط.")

    if st.button("اختبار اتصال Gemini", use_container_width=True):
        _test_key = _get_gemini_api_key()
        if not _test_key:
            st.error("أضف المفتاح أولًا.")
        else:
            try:
                with st.spinner("جارٍ اختبار الاتصال..."):
                    _used = _gemini_connection_test(_test_key, _gemini_model)
                st.success(f"تم الاتصال بنجاح عبر {_used}.")
            except GeminiAPIError as _gem_exc:
                st.error(str(_gem_exc))
    ...
    <div class="hero-grid">
        ...
        <div class="hero-text">
            ...
            <div class="hero-subtitle">
                مراجعة أكاديمية بمساعدة Gemini API مع محرك داخلي مساند للنصوص الإنجليزية،
            ...
            <div class="hero-badges">
                ...
                <span class="hero-badge">🤖 مراجعة تلقائية عبر Gemini API</span>
            </div>
        </div>
    </div>
</div>
...
_banner_msg = (
    "🎉 <b>الإصدار Gemini:</b> مراجعة أكاديمية مدعومة بـGemini API "
...
<div class="features-wrap">
    ...
    <div class="features-grid">
        <div class="feature-card">
            ...
            <strong>مراجعة Gemini مدمجة</strong>
            <span>يحلل المحرك المحلي النص أولًا، ثم يرسل مقتطفات مختارة بلا مراجع إلى Gemini لإصدار مراجعة تفسيرية وأسباب ومقاطع تحتاج تحققًا بشريًا.</span>
        </div>
        <div class="feature-card">
            <div class="feature-icon">📑</div>
...
st.info("📌 هذه النسخة تفحص النصوص الإنجليزية فقط وتستخدم Gemini API إلزاميًا في النتيجة. تُرسل مقتطفات مختارة بعد استبعاد المراجع، ولا تمثل النتيجة نسبة اقتباس أو دليلًا قاطعًا على استخدام الذكاء الاصطناعي.")
...
if analyze_btn and input_text and input_text.strip():
    ...
    if not _active_gemini_key:
        st.session_state.pop("last_result", None)
        st.error("لا يمكن بدء الفحص دون Gemini API Key. أضفه في Streamlit Secrets أو في الحقل السري الجانبي.")
    elif not st.session_state.get("gemini_data_consent", False):
        st.session_state.pop("last_result", None)
        st.error("يجب الموافقة من الشريط الجانبي على إرسال مقتطفات مختارة إلى Gemini قبل بدء الفحص.")
    else:
        try:
            with st.spinner("جارٍ تنفيذ الفحص الداخلي ثم مراجعة النتائج عبر Gemini API..."):
                ...
                result["gemini_review"] = _gemini_assisted_analysis(
                    ...
                    result,
                    _active_gemini_key,
                    st.session_state.get("gemini_model_selector", GEMINI_DEFAULT_MODEL),
                )
            ...
            st.session_state.pop("last_result", None)
            st.error(f"فشل تحليل Gemini: {exc}")
        except Exception as exc:
            st.session_state.pop("last_result", None)
...
if "last_result" in st.session_state:
    st.markdown("### نتيجة المراجعة الأكاديمية بمساعدة Gemini")
    r = st.session_state["last_result"]
    local_pct = r["percentage"]
    gemini_review = r.get("gemini_review") or {}
    pct = float(gemini_review.get("review_priority_score", local_pct))
    clr = gemini_review.get("color") or r["color"]
    pct_label = f"{int(round(pct))}/100"
    meter_width = max(0.0, min(100.0, pct))
    main_verdict = gemini_review.get("verdict_ar") or r["verdict"]
    confidence_label = {"low": "منخفضة", "medium": "متوسطة", "high": "مرتفعة"}.get(
        gemini_review.get("confidence"), "غير محددة"
    )
    ...
    <div class="meter-wrap">
        ...
        <div class="meter-human">ثقة مراجعة Gemini: {confidence_label}</div>
        <div class="bar-track">
    ...
    st.warning(
        "الرقم الظاهر هو درجة أولوية للمراجعة بمساعدة Gemini، وليس نسبة اقتباس، "
        "وليس احتمالًا علميًا لإثبات أن الذكاء الاصطناعي كتب النص."
    )

    if gemini_review.get("available"):
        st.markdown("#### تفسير Gemini")
        st.write(gemini_review.get("summary_ar", ""))
        _gc1, _gc2, _gc3 = st.columns(3)
        _gc1.metric("مخاطر دعم الاستشهادات", f"{gemini_review.get('citation_integrity_risk_score', 0)}/100")
        _gc2.metric("المقاطع التي راجعها Gemini", gemini_review.get("passages_reviewed", 0))
        _gc3.metric("المقاطع المحددة", len(gemini_review.get("flagged_passages") or []))
        ...
        if _flags:
            with st.expander("المقاطع التي طلب Gemini مراجعتها", expanded=True):
                for _flag in _flags:
                    st.markdown(
    ...
    with st.expander("المؤشر المحلي المساند — ليس النتيجة الرئيسية"):
        st.write(f"المؤشر الداخلي: {_format_percentage(local_pct, mask_low=False)}")
        st.write(f"كفاية الأدلة المحلية: {r.get('confidence_percentage', 0):.0f}%")
        st.caption("أُرسل ملخص هذا المؤشر إلى Gemini باعتباره إشارة ضعيفة قابلة للتصحيح، لا باعتباره حقيقة.")
    ...
    if (gemini_review.get("flagged_passages") or []):
        st.info(
            "أضاف Gemini قائمة مستقلة بالمقاطع التي تحتاج مراجعة. تظليل الملف الأصلي يظل أداة مساعدة "
            "ويُراجع مع أسباب Gemini ولا يُعامل كإثبات اقتباس."
        )
    else:
        st.success("لم يحدد Gemini مقاطع بعينها للمراجعة في العينة المرسلة.")
    ...
    st.markdown(
        ...
        f"Gemini review priority: {pct_label} | Local evidence sufficiency: {r['confidence_percentage']:.0f}%"
        f"</div>",
        unsafe_allow_html=True,
    )
